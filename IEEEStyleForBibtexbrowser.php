	...
	if ($bibentry->hasField(Q_AUTHOR)) {
		...
		if (IEEEStyle_FullAuthorList === true and count($authors) > 3)
			...
			if (count($authors) > 5)
				...
				$author = $authors[0] . ', ' . $authors[1] . ', ' . $authors[2] . ' and ' . $authors[3];
...
//		$title = '<a class="bibtitlelink"' . (BIBTEXBROWSER_BIB_IN_NEW_WINDOW? ' target="_blank" ' : '') . ' href="' . $bibentry->getField("url") . '">' . $title . '</a>';
	...
	switch ($entryType) {
		case 'article':
			if ($title) $entry[] = '<b>' . $title . '</b>' ;
			if ($author) $entry[] = '<br>' . $author ;
			if ($journal) $entry[] = '<br>' . $journal ;
		...
		case 'book':
			if ($title) $entry[] = '<b>' . $title . '</b>';
			if ($author) $entry[] = '<br>' . $author;
			if ($address && $publisher) $publisher = '<br>' . $address . ': ' . $publisher;
			if ($edition && $publisher) $entry[] = '<br>' . ($editionShort? $editionShort : $edition) . ' ed. ' . $publisher;
			else if ($publisher) $entry[] = '<br>' . $publisher;
			if ($year) $entry[] = $year;
			break;
		case 'booklet':
			break;
		...
		case 'incollection':
			if ($title) $entry[] = '<b>' . $title . '</b>';
			if ($author) $entry[] = '<br>' . $author  ;
			if ($booktitle) $entry[] = '<br>' . $booktitle ;
		...
		case 'licthesis':
			if ($title) $entry[] = '<b>' . $title . '</b>';
			if ($author) $entry[] = '<br>' . $author;
			if ($type) $entry[] = '<br>' . $type;
			else $entry[] = '<br>' .  'Licentiate Thesis';
			if ($school) $entry[] = $school;
			if ($address) $entry[] = $address;
			if ($year) $entry[] = $year;
			break;
